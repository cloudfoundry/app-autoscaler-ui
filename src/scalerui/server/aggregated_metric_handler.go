...
type AggregatedMetricHandler struct {
	config     *config.Config
	logger     lager.Logger
	httpClient *http.Client
}

func NewAggregatedMetricHandler(logger lager.Logger, config *config.Config, httpClient *http.Client) *AggregatedMetricHandler {
	return &AggregatedMetricHandler{
		config:     config,
		logger:     logger.Session("AggregatedMetricHandler"),
		httpClient: httpClient,
	}
}

func (h *AggregatedMetricHandler) GetAggregatedMetricHistories(w http.ResponseWriter, r *http.Request, vars map[string]string) {
	appId := vars["appid"]
	...
	h.logger.Debug("get-aggregated-metric-histories", lager.Data{"appId": appId, "metrictype": metricType, "start": start, "end": end, "page": page, "resultsPerPage": resultsPerPage, "order": order})
	...
	if len(start) == 1 {
		...
		if err != nil {
			h.logger.Error("get-aggregated-metric-histories-parse-start-time", err, lager.Data{"start": start})
		...
		h.logger.Error("get-aggregated-metric-histories-get-start-time", err, lager.Data{"start": start})
		handlers.WriteJSONResponse(w, http.StatusBadRequest, models.ErrorResponse{
			Code:        "Bad-Request",
	...
	if len(end) == 1 {
		...
		if err != nil {
			h.logger.Error("get-aggregated-metric-histories-parse-end-time", err, lager.Data{"end": end})
			handlers.WriteJSONResponse(w, http.StatusBadRequest, models.ErrorResponse{
				Code:        "Bad-Request",
		...
		h.logger.Error("get-aggregated-metric-histories-get-end-time", err, lager.Data{"end": end})
		handlers.WriteJSONResponse(w, http.StatusBadRequest, models.ErrorResponse{
			Code:        "Bad-Request",
	...
	if len(order) == 1 {
		...
		if orderStr != models.DESCSTR && orderStr != models.ASCSTR {
			h.logger.Error("get-aggregated-metric-histories-parse-order", err, lager.Data{"order": order})
			handlers.WriteJSONResponse(w, http.StatusBadRequest, models.ErrorResponse{
				Code:        "Bad-Request",
	...
	var url string
	path, err := routes.ScalerUiRoutes().Get(routes.GetAggregatedMetricHistoriesRouteName).URLPath("appid", appId, "metrictype", metricType)
	if err != nil {
	...
	h.logger.Debug("get-aggregated-metrics-history-url", lager.Data{"url": url})
	req, err := http.NewRequest(http.MethodGet, url, nil)
	if err != nil {
		h.logger.Error("failed-to-get-aggregated-metrics-from-api-server. request-failed", err, lager.Data{"appId": appId, "err": err})
		handlers.WriteJSONResponse(w, http.StatusInternalServerError, models.ErrorResponse{
			Code:        "Internal-Server-Error",
	...
	if err != nil {
		h.logger.Error("failed-to-retrieve-aggregated-metric-from-api-server. request-failed", err, lager.Data{"appId": appId, "start": start, "end": end, "metricType": metricType, "order": order, "err": err})
		handlers.WriteJSONResponse(w, http.StatusInternalServerError, models.ErrorResponse{
			Code:        "Internal-Server-Error",
